...
function SimpleMarkdown({ content }: { content: string }) {
    const html = content
        ...
        .replace(/\!\[(.*?)\]\((.*?)\)/g, '<img src="$2" alt="$1" class="rounded-lg shadow-md my-4" />') // Added image support
        ...
        .split('\n')
        .map(line => {
            if (line.startsWith('<h') || line.startsWith('<blockquote') || line.startsWith('<code') || line.startsWith('<img')) {
                return line;
            }
            if (line.trim() === '') return '';
            return `<p>${line}</p>`;
